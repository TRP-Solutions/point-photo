...
var PointPhoto = (function(){
	...
	function open(elem){
		...
		camera.video.onloadedmetadata = function(){
			...
			camera.video.addEventListener('resize',rotate_camera_modal);
	...
	function rotate_camera_modal(event){
		if(!active_camera){
			event.originalTarget.removeEventListener('resize',rotate_camera_modal);
			return;
		}
