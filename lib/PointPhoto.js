...
function PointPhotoCamera(container, options){
	...
	function init(instance, container, options){
		...
		container.PointPhotoCameraInstance = instance;
...
function PointPhotoImage(container, options){
	...
	var internal = {
		...
		warning: PointPhoto.create_warning(),
		default_state: preset ? 'preset' : 'empty',
		default_preview_url: preset,
...
var PointPhoto = {
	...
	init: function init(options){
		...
		if(options.camera instanceof PointPhotoCamera){
			this.camera = PointPhotoCamera;
		} else if(options.camera instanceof Element){
			this.camera = options.camera.PointPhotoCameraInstance || new PointPhotoCamera(options.camera, options);
		}
		...
		if(album_images){
			album_images.forEach(image => {
				if(!image.PointPhotoImageInstance){
					var instance = new PointPhotoImage(image, {camera: this.camera});
	...
	create_button: function create_button(name, onclick, type, disabled){
	...
	create_icon: function create_icon(name){
		var span = document.createElement('span');
		span.textContent = name;
		return span;
	},
	create_warning: function create_warning(){
		var div = document.createElement('div');
		div.classList.add('pointphoto-warning');
		div.replaceChildren(PointPhoto.create_icon('warning'));
		return div;
	},
	create_camera_loading: function create_camera_loading(){
		var div = document.createElement('div');
